<?php
        ...
        foreach ($array_database as $k => $value) 
            {
                ...
                if (!empty($nome_pagina)) {
                    #aggiungo i tasti 'modifica' ed 'elimina' se la variabile non è vuota (non mi servono i tasti modifica/elimina nella homepage)
                    $campi[] = tag('td', [], tag('a', ['href' => $nome_pagina . '.html?edit_id=' .$k], 'Modifica'));
                    $campi[] = tag('td', [], tag('a', ['href' => $nome_pagina . '.html?delete_id=' .$k], 'Elimina'));
                }

                $righe_tabella[] = tag('tr', [], implode($campi));
            }

        # logica condizionale se l'array in input è vuoto o no  
        if ($righe_tabella != []) { 
            $table = tag('table', ['border' => 1], implode($righe_tabella));
            return $table;
        } else { 
            $table = '';
            return $table;
    ...
    function build_tab_prenotazioni ($array_database, $nome_pagina) {
